...
const playSound = (isCorrectAnswer) => {
    ...
    if (currentQuestionNo == 5 && isCorrectAnswer) {
        ...
        audioPath = `assets/sounds/question ${currentQuestionNo <= 5 ? "1-5" : currentQuestionNo
...
const populateAnswers = () => {
    ...
    questionsThemeAudio = new Audio(
        `assets/sounds/question ${currentQuestionNo <= 5 ? "1-5" : currentQuestionNo
        }/question theme.mp3`
    );
    ...
    answerSections.forEach((answerSection, index) => {
        ...
        answerSection.parentElement.addEventListener("click", () => {
            questionsThemeAudio.pause();
            // Added protection so that a player who chooses a different answer will not choose his original answer
            if (!answerSelected) {
                answerSelected = true;
                // Have to retrieve the correct answer again to make sure it does not load the previous correct answer
                const question = currentQuestionNo - 1;
                const correctAnswer = allQuestions[question].correct_answer;
...
callFriendButton.addEventListener("click", () => {
    ...
    friendResponse.innerText = `Friend says "${friendKnowsCorrectAnswer
            ? `I think the answer is ${correctAnswer}`
            : "Bro I don't know the answer"
        }"`;
